...
| `src/lib/scan/parse-helpers` (module) | `src/lib/scan/parse-helpers.ts` | shared unit / name / quantity normaliser — imported by `_parse.ts` above |

There is no `/scans` index page and there is no `/api/inventory/scan/route.ts`. The shared unit / name / quantity coercion helpers (`SCAN_UNITS`, `coerceUnit`, `normalizeName`, `normalizeQuantity`, `parseScanResponse`, plus the `ScanUnit` type) live at `src/lib/scan/parse-helpers.ts`; the bill-scan parser imports `coerceUnit` / `normalizeName` / `normalizeQuantity` / `ScanUnit` from it. `tests/unit/scan-parse-helpers.test.ts` covers it directly. (Pre-Phase-4 this file lived at `src/app/api/inventory/scan/_parse.ts` under a misleading folder; the relocation landed in Phase 4.)
...
## Open questions
- **Shared scan helpers moved in Phase 4.** Previously at the misleading path `src/app/api/inventory/scan/_parse.ts`; now lives at `src/lib/scan/parse-helpers.ts` with the covering unit test renamed to `tests/unit/scan-parse-helpers.test.ts`. The bill-scan parser at `src/app/api/bills/scan/_parse.ts` is the only importer. Noted so future audits don't reopen the relocation question.
...
- **`bill-scan-pending` images are moved to `bill-images` on queued-retry finalise.** Phase 4 flipped the queued-retry path from copy-on-success to move-on-success, so `bill-scan-pending` no longer grows unboundedly for successfully-finalised retries. Still untouched by the discard / cancel / resolve paths (those only stamp `reviewed_at`), so a periodic sweep for rows where `reviewed_at < now() - 30d` is still worth considering for storage hygiene on the abandoned-but-not-finalised branch.
...
| `coerceUnit` / `normalizeName` / `normalizeQuantity` / `parseScanResponse` | `src/lib/scan/parse-helpers.ts` | `tests/unit/scan-parse-helpers.test.ts` | — | — | none | `tests/unit/` |
