...
Every action calls `requireHousehold()` (from `src/lib/auth/require.ts`) to resolve the active Clerk session → profile → household before touching Supabase. Mutating actions call `revalidatePath("/recipes")` (and `revalidatePath("/dashboard")` / `revalidatePath("/recipes/${id}")` where relevant) so the catalog index and meal-plan surface re-render after the change.
...
| `recipes.archived_at` | column | `20260517_001_recipes.sql` | Soft-delete timestamp; `effective_recipes` filters out archived rows. No app-side writer today — the archive/unarchive server actions were removed in Phase 4 cleanup as dead UI; the column remains because `effective_recipes` still keys off it (manual SQL or a future archive UI can still flip it). |
...
## Open questions
- _(No open questions for this feature at the moment.)_
